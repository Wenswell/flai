...
- 优先识别 Bug、崩溃、数据丢失、安全隐患、功能缺陷和已废弃逻辑。
- 检查行为是否符合需求，验证是否覆盖核心变更，避免需求遗漏。
- 检查本次相关文件是否有必要、可精简、可合并、可删除。
- 及时指出明显偏大的文件和可拆分逻辑；清理明显多余或明确不需要的文件。

### 通用项目约束

- 外部接口调用后，必须立即保存进度；出错重试时必须能从已保存进度恢复。
- 关键步骤、用户交互、外部接口调用必须记录，终端实时显示开始、进度和结束。
- 终端输出需要简单美化（例如使用console.table），仅在必要时打印Object。
